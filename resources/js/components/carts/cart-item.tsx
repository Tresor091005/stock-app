...
export function CartItem({ item, updateQuantity, removeFromCart }: CartItemProps) {
    ...
    useEffect(() => {
        if (debouncedQuantity !== item.quantity) {
            const newQuantity = Math.min(debouncedQuantity, maxQuantity);
            updateQuantity(item.product_id, newQuantity);
            if (debouncedQuantity > maxQuantity) {
                setQuantity(newQuantity);
            }
        }
    }, [debouncedQuantity, item.quantity, item.product_id, updateQuantity, maxQuantity]);
    ...
    return (
        <div className="flex items-center gap-4 py-4">
            ...
            <div className="flex-grow">
                ...
                <p className="text-xs text-gray-400">In stock: {availableStock}</p>
                <div className="flex items-center gap-2 mt-2">
                    <Input
                        ...
                        onChange={(e) => {
                            ...
                            setQuantity(Math.max(0, Math.min(value || 0, maxQuantity)));
                        }}
                        className="w-16 h-8"
                        min="0"
                        max={maxQuantity}
